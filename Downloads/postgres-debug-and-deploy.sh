#!/bin/bash
# postgres-debug-and-deploy.sh
# Comprehensive PostgreSQL debugging and dashboard deployment script
# Version: 1.0.0
# Date: 2025-08-04

# Set error handling (but don't exit on errors since this is a debugging script)
set -o pipefail
...
LOG_FILE="/var/log/postgres-debug-deploy-$(date +%Y%m%d-%H%M%S).log"
POSTGRES_CONTAINER="risk-platform-postgres"
NGINX_CONTAINER="risk-platform-nginx"
DOCKER_COMPOSE_FILE="/opt/risk-platform/docker-compose.yml"

# Display header
echo -e "${BLUE}===============================================${NC}"
echo -e "${BLUE}  POSTGRESQL DEBUG AND DASHBOARD DEPLOYMENT    ${NC}"
echo -e "${BLUE}===============================================${NC}"
echo ""
...
run_pg_command() {
    ...
    debug "Running PostgreSQL command as user '$user' on database '$db': $cmd"
    
    if [[ -z "$user" ]]; then
        result=$(docker exec "$POSTGRES_CONTAINER" psql -d "$db" -c "$cmd" 2>&1)
    else
        result=$(docker exec "$POSTGRES_CONTAINER" psql -U "$user" -d "$db" -c "$cmd" 2>&1)
    fi
    
    local exit_code=$?
    
    if [[ $exit_code -eq 0 ]]; then
        debug "Command succeeded with output: $result"
    else
        debug "Command failed (exit code $exit_code) with output: $result"
    fi
...
# Function to test PostgreSQL connection with a user
test_pg_connection() {
    local user="$1"
    local db="${2:-postgres}"
    
    debug "Testing PostgreSQL connection as user '$user' to database '$db'"
    
    if [[ -z "$user" ]]; then
        docker exec "$POSTGRES_CONTAINER" psql -d "$db" -c "SELECT 1" >/dev/null 2>&1
    else
        docker exec "$POSTGRES_CONTAINER" psql -U "$user" -d "$db" -c "SELECT 1" >/dev/null 2>&1
    fi
    
    local exit_code=$?
    
    if [[ $exit_code -eq 0 ]]; then
        success "Connection as '$user' to database '$db' successful"
        return 0
    else
        debug "Connection as '$user' to database '$db' failed"
        return 1
    fi
}

# Function to get current user from PostgreSQL
get_current_pg_user() {
    local user="$1"
    local db="${2:-postgres}"
    local current_user
    
    debug "Attempting to get current PostgreSQL user as '$user' on database '$db'"
    
    if [[ -z "$user" ]]; then
        current_user=$(docker exec "$POSTGRES_CONTAINER" psql -d "$db" -t -c "SELECT current_user" 2>/dev/null | tr -d ' ')
    else
        current_user=$(docker exec "$POSTGRES_CONTAINER" psql -U "$user" -d "$db" -t -c "SELECT current_user" 2>/dev/null | tr -d ' ')
    fi
    
    if [[ -n "$current_user" ]]; then
        debug "Current PostgreSQL user: $current_user"
        echo "$current_user"
        return 0
    else
        debug "Failed to get current PostgreSQL user"
        return 1
    fi
}

# Start main script
log "Starting PostgreSQL debugging and dashboard deployment"
...
if ! docker ps | grep -q "$POSTGRES_CONTAINER"; then
    ...
    if [[ -f "$DOCKER_COMPOSE_FILE" ]]; then
        debug "Found docker-compose file at $DOCKER_COMPOSE_FILE"
        docker-compose -f "$DOCKER_COMPOSE_FILE" up -d postgres || {
            error "Failed to start PostgreSQL container using docker-compose."
            exit 1
        }
    else
...
if ! docker ps | grep -q "$NGINX_CONTAINER"; then
    ...
    if [[ -f "$DOCKER_COMPOSE_FILE" ]]; then
        docker-compose -f "$DOCKER_COMPOSE_FILE" up -d nginx || {
            error "Failed to start Nginx container using docker-compose."
            exit 1
        }
    else
        error "Docker-compose file not found at $DOCKER_COMPOSE_FILE"
...
# Step 2: Gather detailed information about PostgreSQL container
section "GATHERING POSTGRESQL CONTAINER INFORMATION"

# Get container details
log "Getting container details for $POSTGRES_CONTAINER"
CONTAINER_INFO=$(docker inspect "$POSTGRES_CONTAINER")
debug "Container info: $(echo "$CONTAINER_INFO" | head -n 20)..."

# Extract image information
PG_IMAGE=$(echo "$CONTAINER_INFO" | grep -o '"Image": "[^"]*"' | head -1 | cut -d'"' -f4)
log "PostgreSQL image: $PG_IMAGE"

# Extract environment variables
log "Extracting environment variables from container"
ENV_VARS=$(docker inspect --format '{{range .Config.Env}}{{println .}}{{end}}' "$POSTGRES_CONTAINER")
debug "Environment variables: $ENV_VARS"

# Look for PostgreSQL user in environment variables
POSTGRES_USER=$(echo "$ENV_VARS" | grep -E "^POSTGRES_USER=" | cut -d'=' -f2)
POSTGRES_PASSWORD=$(echo "$ENV_VARS" | grep -E "^POSTGRES_PASSWORD=" | cut -d'=' -f2)

if [[ -n "$POSTGRES_USER" ]]; then
    log "Found POSTGRES_USER environment variable: $POSTGRES_USER"
else
    warning "POSTGRES_USER environment variable not found"
fi

# Check container logs for initialization messages
log "Checking container logs for PostgreSQL initialization messages"
PG_LOGS=$(docker logs "$POSTGRES_CONTAINER" 2>&1 | grep -E "role|user|postgres" | tail -n 50)
debug "PostgreSQL logs: $PG_LOGS"

# Look for default user creation in logs
DEFAULT_USER=$(echo "$PG_LOGS" | grep -E "creating role|creating user" | grep -oE "[a-zA-Z0-9_]+" | tail -n 1)
if [[ -n "$DEFAULT_USER" ]]; then
    log "Found potential default user in logs: $DEFAULT_USER"
fi

# Step 3: Try to connect with different users
section "TESTING POSTGRESQL CONNECTIONS"

# Define a list of potential users to try
POTENTIAL_USERS=("postgres" "root" "admin" "$POSTGRES_USER" "$DEFAULT_USER" "risk_platform_app" "risk_platform")
WORKING_USER=""
WORKING_DB="postgres"

log "Testing connection with various PostgreSQL users"

# First, try to connect without specifying a user
log "Testing connection without specifying a user"
if test_pg_connection "" "postgres"; then
    CURRENT_USER=$(get_current_pg_user "" "postgres")
    if [[ -n "$CURRENT_USER" ]]; then
        success "Successfully connected without specifying user. Current user: $CURRENT_USER"
        WORKING_USER="$CURRENT_USER"
    else
        warning "Connected without user but couldn't determine current user"
    fi
else
    warning "Could not connect without specifying a user"
fi

# If we don't have a working user yet, try each potential user
if [[ -z "$WORKING_USER" ]]; then
    for user in "${POTENTIAL_USERS[@]}"; do
        ...
        log "Testing connection as user: $user"
        if test_pg_connection "$user" "postgres"; then
            WORKING_USER="$user"
            success "Found working PostgreSQL user: $WORKING_USER"
            break
        fi
    done
fi

# If we still don't have a working user, try connecting to template1 database
if [[ -z "$WORKING_USER" ]]; then
    log "Trying connection to template1 database without user"
    if test_pg_connection "" "template1"; then
        CURRENT_USER=$(get_current_pg_user "" "template1")
        if [[ -n "$CURRENT_USER" ]]; then
            success "Successfully connected to template1 without specifying user. Current user: $CURRENT_USER"
            WORKING_USER="$CURRENT_USER"
            WORKING_DB="template1"
        else
            warning "Connected to template1 without user but couldn't determine current user"
        fi
    else
        warning "Could not connect to template1 without specifying a user"
        
        # Try each potential user with template1
        for user in "${POTENTIAL_USERS[@]}"; do
            if [[ -z "$user" ]]; then
                continue
            fi
            
            log "Testing connection to template1 as user: $user"
            if test_pg_connection "$user" "template1"; then
                WORKING_USER="$user"
                WORKING_DB="template1"
                success "Found working PostgreSQL user for template1: $WORKING_USER"
                break
            fi
        done
    fi
...
# If we still don't have a working user, try more advanced methods
if [[ -z "$WORKING_USER" ]]; then
    section "ADVANCED POSTGRESQL USER DETECTION"
    
    # Try to get PostgreSQL version to determine if PostgreSQL is working at all
    log "Checking PostgreSQL version"
    PG_VERSION=$(docker exec "$POSTGRES_CONTAINER" psql --version 2>&1)
    debug "PostgreSQL version: $PG_VERSION"
    
    # Try to find users by examining PostgreSQL data directory
    log "Examining PostgreSQL data directory"
    PG_HBA_CONF=$(docker exec "$POSTGRES_CONTAINER" cat /var/lib/postgresql/data/pg_hba.conf 2>&1 || echo "Not found")
    debug "pg_hba.conf: $(echo "$PG_HBA_CONF" | head -n 20)..."
    
    # Try to connect as postgres user with POSTGRES_PASSWORD
    if [[ -n "$POSTGRES_PASSWORD" ]]; then
        log "Trying to connect with POSTGRES_PASSWORD environment variable"
        if docker exec -e PGPASSWORD="$POSTGRES_PASSWORD" "$POSTGRES_CONTAINER" psql -U postgres -c "SELECT 1" >/dev/null 2>&1; then
            ...
            success "Successfully connected as postgres user with POSTGRES_PASSWORD"
        else
            warning "Could not connect as postgres user with POSTGRES_PASSWORD"
        fi
    fi
    
    # Try to connect with peer authentication
    log "Trying peer authentication"
    if docker exec "$POSTGRES_CONTAINER" su -c "psql -c 'SELECT 1'" postgres >/dev/null 2>&1; then
        WORKING_USER="postgres"
        success "Successfully connected as postgres user with peer authentication"
    else
        warning "Could not connect with peer authentication"
    fi
    
    # Try to find users by examining PostgreSQL roles
    log "Examining PostgreSQL roles using psql meta-commands"
    ROLES_OUTPUT=$(docker exec "$POSTGRES_CONTAINER" psql -c '\du' 2>&1 || echo "Failed")
    debug "Roles output: $ROLES_OUTPUT"
    
    # Try to connect as the container's default user
    log "Trying to connect as the container's default user"
    CONTAINER_USER=$(docker exec "$POSTGRES_CONTAINER" whoami 2>&1)
    debug "Container default user: $CONTAINER_USER"
    
    if [[ -n "$CONTAINER_USER" ]]; then
        if docker exec "$POSTGRES_CONTAINER" su -c "psql -c 'SELECT 1'" "$CONTAINER_USER" >/dev/null 2>&1; then
            WORKING_USER="$CONTAINER_USER"
            success "Successfully connected as container's default user: $CONTAINER_USER"
        else
            warning "Could not connect as container's default user: $CONTAINER_USER"
        fi
    fi
fi

# Final check if we have a working user
if [[ -z "$WORKING_USER" ]]; then
    error "Could not find a working PostgreSQL user after all attempts."
    error "Please check your PostgreSQL container configuration."
    
    # Provide troubleshooting guidance
    echo -e "\n${YELLOW}Troubleshooting Guidance:${NC}"
    echo "1. Check if PostgreSQL is properly initialized in the container"
    echo "2. Verify that the container has the correct environment variables"
    echo "3. Check if PostgreSQL is listening on the default port"
    echo "4. Examine container logs for authentication errors"
    echo "5. Try restarting the PostgreSQL container"
    echo -e "\nDetailed logs are available at: $LOG_FILE"
    
    exit 1
fi

# Step 4: Database setup with the working user
section "SETTING UP DATABASE WITH USER: $WORKING_USER"

# Check if risk_platform database exists
log "Checking if risk_platform database exists"
DB_EXISTS=$(run_pg_command "$WORKING_USER" "$WORKING_DB" "\l" | grep -c "risk_platform" || echo "0")

if [[ "$DB_EXISTS" -eq "0" ]]; then
    log "Creating risk_platform database"
    if ! run_pg_command "$WORKING_USER" "$WORKING_DB" "CREATE DATABASE risk_platform;" > /dev/null; then
        warning "Failed to create database with standard command. Trying with template0..."
        
        if ! run_pg_command "$WORKING_USER" "$WORKING_DB" "CREATE DATABASE risk_platform WITH TEMPLATE template0 ENCODING 'UTF8';" > /dev/null; then
            warning "Failed with template0. Trying with template1..."
            
            if ! run_pg_command "$WORKING_USER" "$WORKING_DB" "CREATE DATABASE risk_platform WITH TEMPLATE template1 ENCODING 'UTF8';" > /dev/null; then
                error "Failed to create database with all methods. Please check PostgreSQL logs."
                exit 1
            fi
        fi
    fi
...
-- Check if pgcrypto is available, if not use MD5 as fallback
DO $$
BEGIN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'risk_platform_app') THEN
...
log "Installing pgcrypto extension"
run_pg_command "$WORKING_USER" "risk_platform" "CREATE EXTENSION IF NOT EXISTS pgcrypto;" >/dev/null || {
    warning "Could not create pgcrypto extension. Password hashing will use MD5 fallback."
}

# Apply schema migration
log "Applying schema migration"
debug "Copying schema migration to container"

# Try multiple methods to apply schema
log "Method 1: Applying schema using pipe"
if cat "$DB_DIR/migrations/001_initial_schema.sql" | docker exec -i "$POSTGRES_CONTAINER" psql -U "$WORKING_USER" -d risk_platform; then
    success "Schema migration applied successfully using pipe method"
else
    warning "Failed to apply schema migration using pipe. Trying with file copy..."
    
    # Try alternative approach
    log "Method 2: Applying schema using file copy"
    docker cp "$DB_DIR/migrations/001_initial_schema.sql" "$POSTGRES_CONTAINER":/tmp/
    if docker exec "$POSTGRES_CONTAINER" psql -U "$WORKING_USER" -d risk_platform -f /tmp/001_initial_schema.sql; then
        success "Schema migration applied successfully using file copy method"
    else
        warning "Failed to apply schema migration with file copy. Trying with split commands..."
        
        # Try splitting the file into smaller chunks
        log "Method 3: Applying schema using split commands"
        
        # Create schema
        run_pg_command "$WORKING_USER" "risk_platform" "CREATE SCHEMA IF NOT EXISTS risk_platform;" || warning "Failed to create schema"
        
        # Set search path
        run_pg_command "$WORKING_USER" "risk_platform" "SET search_path TO risk_platform;" || warning "Failed to set search path"
        
        # Create basic tables
        log "Creating organizations table"
        run_pg_command "$WORKING_USER" "risk_platform" "
        CREATE TABLE IF NOT EXISTS risk_platform.organizations (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            name VARCHAR(255) NOT NULL,
            slug VARCHAR(100) UNIQUE NOT NULL,
            industry VARCHAR(100),
            ...
            updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            deleted_at TIMESTAMP WITH TIME ZONE
        );" || warning "Failed to create organizations table"
        
        log "Creating users table"
        run_pg_command "$WORKING_USER" "risk_platform" "
        CREATE TABLE IF NOT EXISTS risk_platform.users (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            ...
            updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
            deleted_at TIMESTAMP WITH TIME ZONE
        );" || warning "Failed to create users table"
        ...
        VALUES ('Default Organization', 'default', 'Technology')
        ON CONFLICT (slug) DO NOTHING;" || warning "Failed to create default organization"
        ...
        INSERT INTO risk_platform.users (
            ...
            'active',
            NOW()
        ) ON CONFLICT (email) DO NOTHING;" || warning "Failed to create admin user"
        
        success "Basic schema applied using split commands method"
    fi
fi

# Create database users
log "Creating database users"
if cat "$DB_DIR/create_users.sql" | docker exec -i "$POSTGRES_CONTAINER" psql -U "$WORKING_USER" -d risk_platform; then
    success "Database users created successfully"
else
    warning "Could not create database users. Trying with file copy..."
    
    # Try alternative approach
    docker cp "$DB_DIR/create_users.sql" "$POSTGRES_CONTAINER":/tmp/
    if docker exec "$POSTGRES_CONTAINER" psql -U "$WORKING_USER" -d risk_platform -f /tmp/create_users.sql; then
        success "Database users created successfully using file copy method"
    else
        warning "Could not create database users with all methods. Continuing with current user."
    fi
fi

# Step 5: Create dashboard frontend
section "CREATING DASHBOARD FRONTEND"
...
# Step 6: Configure Nginx to serve the dashboard
section "CONFIGURING NGINX TO SERVE THE DASHBOARD"
...
if docker ps | grep -q "$NGINX_CONTAINER"; then
    ...
    log "Copying dashboard files to Nginx container"
    
    # Try multiple methods to copy files
    log "Method 1: Creating directory in container"
    if docker exec "$NGINX_CONTAINER" mkdir -p /opt/risk-platform/dashboard/public; then
        log "Directory created successfully"
        
        log "Method 2: Copying files using docker cp"
        if docker cp "$DASHBOARD_DIR/public" "$NGINX_CONTAINER":/opt/risk-platform/dashboard/; then
            success "Files copied successfully using docker cp"
        else
            warning "Docker cp failed, trying alternative method"
            
            log "Method 3: Copying files one by one"
            for file in "$DASHBOARD_DIR/public/"*; do
                filename=$(basename "$file")
                if cat "$file" | docker exec -i "$NGINX_CONTAINER" sh -c "cat > /opt/risk-platform/dashboard/public/$filename"; then
                    debug "Copied file: $filename"
                else
                    warning "Failed to copy file: $filename"
                fi
            done
            success "Files copied one by one"
        fi
    else
        warning "Creating directory in container failed, trying with docker cp directly"
        
        if docker cp "$DASHBOARD_DIR/public" "$NGINX_CONTAINER":/opt/risk-platform/dashboard/; then
            success "Files copied successfully using docker cp"
        else
            error "Failed to copy dashboard files to Nginx container"
            exit 1
        fi
    fi
    
    # Copy Nginx configuration to container
    log "Copying Nginx configuration to container"
    if docker cp /tmp/dashboard.conf "$NGINX_CONTAINER":/etc/nginx/conf.d/default.conf; then
        success "Nginx configuration copied successfully"
    else
        warning "Docker cp for Nginx config failed, trying alternative approach"
        
        if cat /tmp/dashboard.conf | docker exec -i "$NGINX_CONTAINER" sh -c "cat > /etc/nginx/conf.d/default.conf"; then
            success "Nginx configuration copied successfully using alternative method"
        else
            error "Failed to copy Nginx configuration to container"
            exit 1
        fi
    fi
    ...
    log "Copying dashboard files to Nginx directory"
    mkdir -p /var/www/risk-platform
    if cp -r "$DASHBOARD_DIR/public" /var/www/risk-platform/; then
        success "Dashboard files copied to Nginx directory"
    else
        error "Failed to copy dashboard files to Nginx directory"
        exit 1
    fi
    
    # Create Nginx configuration
    log "Creating Nginx configuration"
    if echo "$NGINX_CONF" > /etc/nginx/sites-available/risk-platform; then
    ...
    if ln -sf /etc/nginx/sites-available/risk-platform /etc/nginx/sites-enabled/; then
...
# Step 7: Create enterprise scripts
section "CREATING ENTERPRISE SCRIPTS"
...
mkdir -p "$(dirname "$THREAT_FEEDS_LOG")"
...
db_user = os.environ.get('DB_USER', 'risk_platform_app')
...
try:
    ...
    print(f"Error updating MITRE ATT&CK data: {e}")
PYTHON_EOF
...
# Get database user
DB_USER=$(docker exec "$POSTGRES_CONTAINER" psql -c "\du" 2>/dev/null | grep -E "Superuser|superuser" | head -1 | awk '{print $1}')
if [[ -z "$DB_USER" ]]; then
    DB_USER="risk_platform_app"
fi

# Create user in database
docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform -c "
...
ORPHANED_USERS=$(docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform -t -c "
...
log "Testing final PostgreSQL connection"
if test_pg_connection "$WORKING_USER" "risk_platform"; then
    success "PostgreSQL connection successful"
else
    warning "PostgreSQL connection test failed. Please check database logs."
fi

# Check if dashboard files are accessible
log "Checking if dashboard files are accessible"
if docker exec "$NGINX_CONTAINER" ls -la /opt/risk-platform/dashboard/public/ >/dev/null 2>&1; then
    success "Dashboard files are accessible in Nginx container"
else
    warning "Dashboard files are not accessible in Nginx container"
fi
...
echo -e "Working User: ${YELLOW}$WORKING_USER${NC}"
echo -e "Working Database: ${YELLOW}risk_platform${NC}"
echo ""
