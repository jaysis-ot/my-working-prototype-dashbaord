#!/bin/bash
# fix-postgres-deploy-dashboard.sh
# Script to fix PostgreSQL user issues and deploy the Risk Platform dashboard
# Version: 1.0.0
# Date: 2025-08-04
...
# Step 2: Identify PostgreSQL users and fix database issues
log "Step 2: Identifying PostgreSQL users and fixing database issues"

# Check what users exist in PostgreSQL container
log "Checking PostgreSQL users..."
PG_USERS=$(docker exec "$POSTGRES_CONTAINER" psql -c "\du" 2>/dev/null || echo "Failed to list users")

# If we can't list users, try to find the default user
if [[ "$PG_USERS" == "Failed to list users" ]]; then
    log "Could not list PostgreSQL users with default command. Trying alternative approaches..."
    
    # Try to find the default user by checking environment variables
    DB_USER=$(docker inspect "$POSTGRES_CONTAINER" | grep POSTGRES_USER | head -1 | awk -F'"' '{print $4}')
    if [[ -z "$DB_USER" ]]; then
        # If POSTGRES_USER is not set, check common default users
        for user in postgres root admin risk_platform_app; do
            if docker exec "$POSTGRES_CONTAINER" psql -U "$user" -c "SELECT 1" >/dev/null 2>&1; then
                DB_USER="$user"
                log "Found working PostgreSQL user: $DB_USER"
                break
            fi
        done
    else
        log "Found PostgreSQL user from environment: $DB_USER"
    fi
    
    # If we still don't have a user, check if we can connect without specifying a user
    if [[ -z "$DB_USER" ]]; then
        if docker exec "$POSTGRES_CONTAINER" psql -c "SELECT 1" >/dev/null 2>&1; then
            DB_USER=$(docker exec "$POSTGRES_CONTAINER" psql -c "SELECT current_user" -t | tr -d ' ')
            log "Connected to PostgreSQL without specifying user. Current user: $DB_USER"
        else
            error "Could not determine PostgreSQL user. Please check container configuration."
            exit 1
        fi
    fi
else
    # Parse the output to find superuser
    DB_USER=$(echo "$PG_USERS" | grep -E "Superuser|superuser" | head -1 | awk '{print $1}')
    if [[ -z "$DB_USER" ]]; then
        # If no superuser found, use any user
        DB_USER=$(echo "$PG_USERS" | grep -v "Role name" | head -1 | awk '{print $1}')
    fi
    log "Found PostgreSQL user: $DB_USER"
fi

# Check if the database exists
log "Checking if risk_platform database exists..."
DB_EXISTS=$(docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -lqt | grep -c "risk_platform" || echo "0")

if [[ "$DB_EXISTS" -eq "0" ]]; then
    log "Creating risk_platform database..."
    docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -c "CREATE DATABASE risk_platform;" || {
        error "Failed to create database. Trying alternative approach..."
        # Try creating database with template0
        docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -c "CREATE DATABASE risk_platform WITH TEMPLATE template0 ENCODING 'UTF8';" || {
            error "Failed to create database with template0. Trying with template1..."
            docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -c "CREATE DATABASE risk_platform WITH TEMPLATE template1 ENCODING 'UTF8';" || {
...
docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform -c "CREATE EXTENSION IF NOT EXISTS pgcrypto;" || {
    warning "Could not create pgcrypto extension. Password hashing will use MD5 fallback."
}

# Apply schema migration
log "Applying schema migration"
cat "$DB_DIR/migrations/001_initial_schema.sql" | docker exec -i "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform || {
    error "Failed to apply schema migration. Trying with -f option..."
    # Try alternative approach
    docker cp "$DB_DIR/migrations/001_initial_schema.sql" "$POSTGRES_CONTAINER":/tmp/
    docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform -f /tmp/001_initial_schema.sql || {
        error "Failed to apply schema migration with all methods. Please check PostgreSQL logs."
        exit 1
    }
}
...
cat "$DB_DIR/create_users.sql" | docker exec -i "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform || {
    warning "Could not create database users. Trying with -f option..."
    # Try alternative approach
    docker cp "$DB_DIR/create_users.sql" "$POSTGRES_CONTAINER":/tmp/
    docker exec "$POSTGRES_CONTAINER" psql -U "$DB_USER" -d risk_platform -f /tmp/create_users.sql || {
...
if docker ps | grep -q "$NGINX_CONTAINER"; then
    ...
    docker exec "$NGINX_CONTAINER" mkdir -p /opt/risk-platform/dashboard/ || {
        log "Creating directory in container failed, trying with docker cp"
        docker cp "$DASHBOARD_DIR/public" "$NGINX_CONTAINER":/opt/risk-platform/dashboard/
    }
    
    # If docker cp failed, try alternative approach
    if [ $? -ne 0 ]; then
        log "Docker cp failed, trying to mount volume"
        # Create directory in container
        docker exec "$NGINX_CONTAINER" mkdir -p /opt/risk-platform/dashboard/public
        
        # Copy files one by one
        for file in "$DASHBOARD_DIR/public/"*; do
            filename=$(basename "$file")
            cat "$file" | docker exec -i "$NGINX_CONTAINER" sh -c "cat > /opt/risk-platform/dashboard/public/$filename"
        done
    fi
    
    # Copy Nginx configuration to container
    docker cp /tmp/dashboard.conf "$NGINX_CONTAINER":/etc/nginx/conf.d/default.conf || {
        log "Docker cp for Nginx config failed, trying alternative approach"
        cat /tmp/dashboard.conf | docker exec -i "$NGINX_CONTAINER" sh -c "cat > /etc/nginx/conf.d/default.conf"
    }
