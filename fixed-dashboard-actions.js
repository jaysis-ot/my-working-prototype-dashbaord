...
export const ACTION_TYPES = {
  ...
  RESET_FRAMEWORK_DATA: 'RESET_FRAMEWORK_DATA',
...
const validateFramework = (framework) => {
  const validFrameworks = ['nistCsf', 'iso27001', 'soc2', 'pciDss'];
  
  if (!validFrameworks.includes(framework)) {
    console.warn(`Invalid framework: ${framework}`);
    return false;
  }
...
export const dashboardActions = {
  ...
  /**
   * Update standards assessment data
   */
  updateStandardsData: (framework, data) => {
    if (!validateFramework(framework)) {
      return { type: '@@INVALID_ACTION' };
    }
    
    return createAction(ACTION_TYPES.UPDATE_STANDARDS_DATA, { 
  ...
  setSelectedFramework: (framework) => {
    ...
    return createAction(ACTION_TYPES.SET_SELECTED_FRAMEWORK, { 
      payload: { framework } 
    }, { 
      category: 'standards',
      framework
   ...
   */
  resetFrameworkData: (framework) => {
    if (!validateFramework(framework)) {
      return { type: '@@INVALID_ACTION' };
    }
    
    return createAction(ACTION_TYPES.RESET_FRAMEWORK_DATA, { 
      payload: { framework } 
    }, { 
      category: 'standards',
      framework
   ...
   */
  exportStandardsData: (framework, format = 'json') => createAction(ACTION_TYPES.EXPORT_STANDARDS_DATA, { 
    payload: { framework, format } 
  }, { 
    category: 'standards',
    framework,
    format
   ...
   */
  importStandardsData: (framework, data, metadata = {}) => createAction(ACTION_TYPES.IMPORT_STANDARDS_DATA, { 
    payload: { framework, data, metadata } 
  }, { 
    category: 'standards',
    framework,
    importSize: JSON.stringify(data || {}).length
  }),
...
export const complexActions = {
  ...
  /**
   * ✅ Navigate to Standards view with framework selection
   */
  navigateToStandards: (framework = null) => (dispatch) => {
    // Navigate to standards view
    dispatch(dashboardActions.setViewMode(VIEW_MODES.STANDARDS));
    
    // Set framework if provided
    if (framework) {
      dispatch(dashboardActions.setSelectedFramework(framework));
    }
    
    // Track the navigation
    dispatch(dashboardActions.trackPageView(VIEW_MODES.STANDARDS, { framework }));
  },

  /**
   * ✅ Update framework assessment with progress calculation
   */
  updateFrameworkAssessment: (framework, assessmentData, additionalData = {}) => (dispatch) => {
    ...
    const overallScore = Object.values(assessmentData).reduce((sum, item) => {
      const itemScore = ((item.maturity || 0) + (item.implementation || 0) + (item.evidence || 0)) / 3;
      return sum + itemScore;
    }, 0) / totalItems * 100 / 3; // Normalize to 0-100

    // Update standards data
    dispatch(dashboardActions.updateStandardsData(framework, {
      assessmentData,
      completionRate,
      overallScore,
      ...additionalData
    ...
    // Track the assessment update
    dispatch(dashboardActions.trackUserAction('updateFrameworkAssessment', { 
      framework, 
